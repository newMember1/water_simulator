...
vec3 lightDirec = vec3(0.0, -1.0, 0.8);
...
{
    e.z = (max(e.z, 0.0) * 0.8 + 0.2) * 0.8;
    return vec3(pow(1.0 - e.z,2.0), 1.0 - e.z, 0.6 + (1.0 - e.z)*0.4) * 1.1;
}
...
vec3 getSeaColor(vec3 pos,vec3 normal, vec3 lightDir, vec3 viewDir)
{
    ...
    vec3 refracted = seaBase + diffuse(normal, lightDir, 200.0) * seaWaterColor * 0.2;

    //mix reflected and refracted with fresnel
    vec3 color = mix(refracted, reflected, fresnel);

    //add specular
    color += vec3(specular(normal, lightDir, viewDir,100.0));
    //color =  vec3(specular(normal, -lightDir, viewDir,200.0));
    return color;
}
...
void main(void)
{
    ...
    fColor = vec4(getSeaColor(originPos, originNormal, lightDirec, viewDirec),1.0);
